    ...
    def populate_initial_data(self):
        ...
        if cursor.fetchone()[0] == 0:
            ...
            categories = [
                ...
                ("Алгоритми та структури даних", "Алгоритми сортування, пошуку, структури даних"),
...
class AuthenticationManager:
    ...
    def login_user(self, username: str, password: str) -> bool:
        ...
        if result:
            self.current_user = User(result[0], result[1], result[2], bool(result[3]))
            return True
        return False
...
class TestManager:
    ...
    def start_test(self, category_id: int, num_questions: int = 10) -> bool:
        ...
        for q_data in questions_data:
            ...
            question = Question(q_data[0], q_data[1], q_data[2], q_data[3], q_data[4], options, q_data[6], q_data[7])
...
class InformaticsTrainerGUI:
    ...
    def show_login_screen(self):
        ...
        # Заголовок
        title_label = ttk.Label(main_frame, text="Тренажер з Інформатики", style='Title.TLabel')
        title_label.pack(pady=(0, 30))
        
        # Фрейм для форми входу
        login_frame = ttk.LabelFrame(main_frame, text="Вхід в систему", padding="20")
        ...
        ttk.Button(button_frame, text="Увійти", command=self.login, style='Custom.TButton').pack(side='left', padx=(0, 10))
        ttk.Button(button_frame, text="Реєстрація", command=self.show_register_screen, style='Custom.TButton').pack(side='left')
        
        # Повідомлення
        self.message_label = ttk.Label(login_frame, text="", style='Error.TLabel')
        self.message_label.pack(pady=10)
        
        # Демо-дані
        demo_frame = ttk.LabelFrame(main_frame, text="Демо-доступ", padding="10")
        demo_frame.pack(pady=10, padx=50, fill='x')
        
        ttk.Label(demo_frame, text="Адміністратор: admin / admin123").pack()
        ttk.Button(demo_frame, text="Увійти як адмін", command=self.demo_admin_login).pack(pady=5)
    ...
    def show_register_screen(self):
        ...
        title_label = ttk.Label(main_frame, text="Реєстрація нового користувача", style='Title.TLabel')
        title_label.pack(pady=(0, 30))
        
        register_frame = ttk.LabelFrame(main_frame, text="Дані користувача", padding="20")
        ...
        ttk.Label(register_frame, text="Email (необов'язково):").pack(anchor='w')
        ...
        ttk.Button(button_frame, text="Зареєструватися", command=self.register).pack(side='left', padx=(0, 10))
        ttk.Button(button_frame, text="Назад", command=self.show_login_screen).pack(side='left')
    ...
    def login(self):
        ...
        if not username or not password:
            self.message_label.config(text="Заповніть всі поля", style='Error.TLabel')
            return
        
        if self.auth_manager.login_user(username, password):
            self.show_main_menu()
        else:
            self.message_label.config(text="Невірне ім'я користувача або пароль", style='Error.TLabel')
    ...
    def register(self):
        ...
        if not username or not password:
            self.reg_message_label.config(text="Заповніть обов'язкові поля", style='Error.TLabel')
            return
        
        if password != confirm:
            self.reg_message_label.config(text="Паролі не співпадають", style='Error.TLabel')
            return
        
        if len(password) < 6:
            self.reg_message_label.config(text="Пароль повинен містити мінімум 6 символів", style='Error.TLabel')
            return
        
        if self.auth_manager.register_user(username, password, email):
            self.reg_message_label.config(text="Реєстрація успішна! Тепер ви можете увійти", style='Success.TLabel')
            self.root.after(2000, self.show_login_screen)
        else:
            self.reg_message_label.config(text="Користувач з таким ім'ям вже існує", style='Error.TLabel')
    ...
    def show_main_menu(self):
        ...
        title_label = ttk.Label(main_frame, text=welcome_text, style='Title.TLabel')
        title_label.pack(pady=(0, 30))
    ...
    def show_category_selection(self):
        ...
        title_label = ttk.Label(main_frame, text="Виберіть категорію для тестування", style='Title.TLabel')
        ...
        for category in categories:
            category_frame = ttk.LabelFrame(categories_frame, text=category[1], padding="10")
        ...
        # Кнопка назад
        ttk.Button(main_frame, text="Назад до меню", command=self.show_main_menu).pack(pady=20)
    ...
    def start_test(self, category_id: int, num_questions: int):
        ...
        if self.test_manager.start_test(category_id, num_questions):
            ...
            messagebox.showerror("Помилка", "Недостатньо питань в цій категорії")
    ...
    def show_test_question(self):
        ...
        progress_label = ttk.Label(main_frame, text=progress_text, style='Heading.TLabel')
        progress_label.pack(pady=(0, 10))
        
        # Прогрес-бар
        progress_bar = ttk.Progressbar(main_frame, length=400, mode='determinate')
        progress_bar['value'] = ((self.test_manager.current_question_index) / len(self.test_manager.current_questions)) * 100
        progress_bar.pack(pady=(0, 20))
        
        # Питання
        question_frame = ttk.LabelFrame(main_frame, text=f"Складність: {'⭐' * question.difficulty}", padding="20")
        question_frame.pack(fill='both', expand=True, pady=10)
        
        question_label = ttk.Label(question_frame, text=question.question_text, wraplength=600, style='Heading.TLabel')
        ...
        if question.question_type == "multiple_choice":
            ...
            ttk.Label(question_frame, text="Введіть відповідь:").pack(anchor='w')
        ...
        ttk.Button(button_frame, text="Наступне питання", command=self.submit_answer).pack(side='right', padx=(10, 0))
        ttk.Button(button_frame, text="Завершити тест", command=self.finish_test).pack(side='right')
    ...
    def submit_answer(self):
        ...
        if not answer:
            messagebox.showwarning("Увага", "Будь ласка, оберіть або введіть відповідь")
            return
    ...
    def finish_test(self):
        ...
        results = self.test_manager.finish_test(self.auth_manager.current_user.user_id)
    ...
    def show_test_results(self, results: Dict):
        ...
        title_label = ttk.Label(main_frame, text="Результати тестування", style='Title.TLabel')
        title_label.pack(pady=(0, 30))
        
        # Результати
        results_frame = ttk.LabelFrame(main_frame, text="Ваші результати", padding="20")
        ...
        ttk.Label(results_frame, text=f"Оцінка: {grade}", style=grade_style).pack(anchor='w', pady=10)
        
        # Детальний розбір
        if self.test_manager.current_questions:
            details_frame = ttk.LabelFrame(main_frame, text="Детальний розбір", padding="10")
            details_frame.pack(fill='both', expand=True, pady=10)
            
            # Скролюючий текст
            details_text = scrolledtext.ScrolledText(details_frame, height=10, wrap=tk.WORD)
            details_text.pack(fill='both', expand=True)
            
            for i, question in enumerate(self.test_manager.current_questions):
                user_answer = self.test_manager.user_answers[i] if i < len(self.test_manager.user_answers) else "Не відповів"
                is_correct = user_answer.strip().lower() == question.correct_answer.strip().lower()
                
                details_text.insert(tk.END, f"Питання {i+1}: {question.question_text}\n")
                details_text.insert(tk.END, f"Ваша відповідь: {user_answer}\n")
                details_text.insert(tk.END, f"Правильна відповідь: {question.correct_answer}\n")
                details_text.insert(tk.END, f"Результат: {'✅ Правильно' if is_correct else '❌ Неправильно'}\n")
                if question.explanation:
                    details_text.insert(tk.END, f"Пояснення: {question.explanation}\n")
        ...
        ttk.Button(button_frame, text="Пройти ще раз", command=self.show_category_selection).pack(side='left', padx=(0, 10))
        ttk.Button(button_frame, text="Головне меню", command=self.show_main_menu).pack(side='left')
    ...
    def show_results(self):
        ...
        title_label = ttk.Label(main_frame, text="Історія результатів", style='Title.TLabel')
        title_label.pack(pady=(0, 20))
        ...
        if not results:
            ttk.Label(main_frame, text="Ви ще не проходили тестування", style='Heading.TLabel').pack(pady=50)
        else:
            # Таблиця результатів
            columns = ('Дата', 'Категорія', 'Питань', 'Правильно', 'Відсоток', 'Час')
            ...
            for result in results:
                ...
                tree.insert('', 'end', values=(date, category, total, correct, f"{percentage}%", time_spent))
        ...
        ttk.Button(main_frame, text="Назад", command=self.show_main_menu).pack(pady=20)
    ...
    def show_statistics(self):
        ...
        title_label = ttk.Label(main_frame, text="Статистика", style='Title.TLabel')
        title_label.pack(pady=(0, 20))
        ...
        if general_stats[0] == 0:
            ...
            # Загальна статистика
            general_frame = ttk.LabelFrame(main_frame, text="Загальна статистика", padding="15")
            ...
            # Статистика по категоріях
            if category_stats:
                category_frame = ttk.LabelFrame(main_frame, text="Статистика по категоріях", padding="15")
                ...
                for cat_stat in category_stats:
                    ...
                    ttk.Label(category_frame, text=cat_text).pack(anchor='w', pady=2)
    ...
    def show_admin_panel(self):
        ...
        title_label = ttk.Label(main_frame, text="Адміністративна панель", style='Title.TLabel')
        title_label.pack(pady=(0, 20))
    ...
    def show_question_management(self):
        ...
        messagebox.showinfo("Інформація", "Функція управління питаннями буде реалізована в наступній версії")
    
    def show_user_management(self):
        """Управління користувачами"""
        messagebox.showinfo("Інформація", "Функція управління користувачами буде реалізована в наступній версії")
    
    def show_system_statistics(self):
        """Системна статистика"""
        messagebox.showinfo("Інформація", "Функція системної статистики буде реалізована в наступній версії")
    
    def export_data(self):
        """Експорт даних"""
        messagebox.showinfo("Інформація", "Функція експорту даних буде реалізована в наступній версії")
    ...
    def show_settings(self):
        ...
        title_label = ttk.Label(main_frame, text="Налаштування", style='Title.TLabel')
        title_label.pack(pady=(0, 20))
        
        settings_frame = ttk.LabelFrame(main_frame, text="Налаштування профілю", padding="20")
        settings_frame.pack(fill='x', pady=10)
        
        # Інформація про користувача
        user = self.auth_manager.current_user
        ttk.Label(settings_frame, text=f"Ім'я користувача: {user.username}", style='Heading.TLabel').pack(anchor='w', pady=5)
        ttk.Label(settings_frame, text=f"Email: {user.email or 'Не вказано'}", style='Heading.TLabel').pack(anchor='w', pady=5)
        ttk.Label(settings_frame, text=f"Тип акаунту: {'Адміністратор' if user.is_admin else 'Користувач'}", 
                 style='Heading.TLabel').pack(anchor='w', pady=5)
        
        # Кнопки налаштувань
        ttk.Button(settings_frame, text="Змінити пароль", command=self.change_password).pack(pady=10)
        
        ttk.Button(main_frame, text="Назад", command=self.show_main_menu).pack(pady=20)
    
    def change_password(self):
        """Зміна паролю"""
        messagebox.showinfo("Інформація", "Функція зміни паролю буде реалізована в наступній версії")
